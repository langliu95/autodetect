...
if __name__ == '__main__':
    ...
    num = args.num
    if args.shuffle:
        print("repetition {}".format(num))
        stat = np.zeros((l, l))
        tau = np.zeros((l, l))
        size1 = np.zeros((l, l))
        for i in range(l):
            print('the first show is {}'.format(NAME[i]))
            for j in range(l):
                print('the second show is {}'.format(NAME[j]))
                while True:
                ...
                tau[i, j] = res[1]
                size1[i, j] = pars[2]
        filename = "tv_shuffle_freq" + str(args.freq) + '_obs' + str(args.obs_per_state) + '_' + str(num) + ".txt"
        np.savetxt(filename, np.row_stack([stat, tau, size1]), delimiter=',')
    else:
        stat, tau, n1 = 0.0, 0, 0
        i = int(num / l)
        j = num % l
        print("pair ({}, {})".format(NAME[i], NAME[j]))
        res, pars = change_detection(i, j, obs_per_state=args.obs_per_state)
        filename = "tv_shows_freq" + str(args.freq) + '_obs' + str(args.obs_per_state) + '_' + str(num) + ".txt"
